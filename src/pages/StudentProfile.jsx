            ...
            <div className="pt-24 pb-10 px-10">
              ...
              <div className="space-y-5 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                <div className="flex justify-between items-center text-base border-b border-slate-200 pb-4">
                  <span className="text-slate-500 font-bold uppercase tracking-wider text-xs">E-Posta</span>
                  <span className="font-medium text-slate-800 truncate w-32 text-right">{currentUser?.email || "Eklenmemiş"}</span>
                </div>
                <div className="flex justify-between items-center text-base border-b border-slate-200 pb-4">
          ...
          {hasActiveExam ? (
            <div className="bg-gradient-to-r from-indigo-600 to-blue-700 rounded-[3rem] p-10 md:p-12 text-white relative overflow-hidden shadow-2xl shadow-indigo-500/30">
                ...
                <div className="col-span-2 md:col-span-3 mt-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 mt-6">
                      {neighborhoodDetails.mapLink && (
                          <a href={neighborhoodDetails.mapLink} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center text-sm font-black bg-indigo-500/50 px-5 py-3 rounded-xl hover:bg-indigo-400 transition border border-indigo-400/50">
                              <Map className="w-5 h-5 mr-2"/> Haritada Konumu Aç
                          </a>
                      )}
                      <a href={`tel:${neighborhoodDetails.phone}`} className="inline-flex items-center justify-center text-sm font-black bg-indigo-500/50 px-5 py-3 rounded-xl hover:bg-indigo-400 transition border border-indigo-400/50">
                          <Phone className="w-5 h-5 mr-2"/> {neighborhoodDetails.contactName ? `${neighborhoodDetails.contactName}: ` : ''}{neighborhoodDetails.phone}
                      </a>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center relative z-10 gap-6 mt-10 border-t border-indigo-500/30 pt-8">
                ...
                {!isExamTimePassed && (
                  <button onClick={() => navigateTo('register')} className="w-full sm:w-auto bg-white/20 text-white px-8 py-4 rounded-2xl text-lg font-black shadow-lg hover:bg-white/30 transition flex items-center justify-center border border-white/40">
                    <Clock className="w-6 h-6 mr-3"/> Oturumu Değiştir
                  </button>
                )}
              </div>
